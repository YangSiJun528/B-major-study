...
인프라 장비는 L2/L3만 보는 것이 아니라, 보안과 트래픽 제어를 위해 L4의 protocol/port/flow까지 보는 경우가 많다.   
CCNA에서 L4 인프라 관점은 장비 명령어보다 **트래픽을 어떤 단위로 식별하고 제어하는지**를 이해하는 것이 핵심이다.
...
- 내가 정리한 관련 섹션: [문제 해결 방법론](section_13/ysj/note.md), [ping/traceroute/port 확인](section_18/ysj/note.md), [ACL](section_28/ysj/note.md), [NAT/PAT](section_29/ysj/note.md), [방화벽](section_32/ysj/note.md)

# Part 2. 프로토콜 규칙과 메시지 의미

이 파트는 "어떤 장비를 지나가는가"보다 **각 계층의 메시지가 어떤 의미를 갖고, 양끝이 어떤 규칙으로 해석하는가**에 초점을 둔다.

## L2/L3 메시지 규칙 요약

Part 1에서 L2/L3를 장비와 경로 관점으로 봤다면, 여기서는 프로토콜 메시지의 의미만 짧게 본다.

### Ethernet / MAC

- **Ethernet frame**은 같은 링크/VLAN 안에서 전달되는 L2 단위다.
- **Source/Destination MAC**은 현재 링크에서의 송신자/수신자를 가리킨다.
- 라우터를 넘으면 IP packet은 이어질 수 있지만, Ethernet frame과 MAC 주소는 다음 링크에 맞게 새로 만들어진다.
- **FCS/CRC**는 frame 손상을 검출한다. 손상 복구와 재전송은 보통 상위 계층의 책임이다.

### ARP / ND

- **ARP(IPv4)**: 같은 L2 영역에서 next-hop IP에 대응하는 MAC 주소를 찾는다.
- **ND(IPv6)**: IPv6에서 ARP 역할을 포함하며, neighbor discovery와 router discovery를 담당한다.
- 둘 다 "최종 목적지의 MAC"을 항상 찾는 것이 아니라, 실제로 다음에 frame을 보낼 **next-hop의 MAC**을 찾는다.

### IP / ICMP

- **IP packet**은 출발지/목적지 IP, TTL/Hop Limit, 상위 프로토콜 번호(TCP/UDP/ICMP 등)를 담는다.
- IP는 connectionless / best-effort라서 순서, 도착, 재전송을 보장하지 않는다.
- **TTL/Hop Limit**은 packet이 라우팅 루프에 빠져 무한히 돌지 않게 하는 제한값이다.
- **ICMP**는 IP 처리 중의 오류나 상태를 알려주는 제어 프로토콜이다. `ping`, `traceroute`, `Destination Unreachable`, `Time Exceeded`를 볼 때 핵심이다.

### MTU / Fragmentation

- **MTU(Maximum Transmission Unit)**: 한 링크에서 한 번에 실을 수 있는 최대 L3 packet 크기
- TCP는 보통 path MTU에 맞춰 **MSS(Maximum Segment Size)**를 정해 segment가 너무 커지지 않게 한다. UDP는 TCP처럼 segment 크기를 자동 조절해주지 않으므로, 애플리케이션/상위 프로토콜이 datagram 크기를 신경 써야 한다.
- IPv4는 fragmentation이 가능하지만 성능/장애 추적 측면에서 피하는 편이 좋고, IPv6에서는 중간 라우터가 fragmentation하지 않는다.
...
- port 번호는 16-bit라 `0~65535` 범위이고, IANA 기준으로 크게 **System/Well-known `0~1023`**, **User/Registered `1024~49151`**, **Dynamic/Private `49152~65535`**로 나뉜다.
  - > **IANA(Internet Assigned Numbers Authority)** 는 port 번호, protocol number, DNS root zone처럼 인터넷에서 전 세계가 공통으로 써야 하는 번호/이름 자원을 관리하는 기관이다.
  - `0~49151`은 IANA가 서비스 용도로 배정/등록할 수 있는 영역이다. 
  - `49152~65535`는 IANA가 특정 서비스에 배정하지 않는 영역이고, 보통 OS가 client의 임시 source port(ephemeral port)로 사용한다.
...
- 내가 정리한 관련 섹션: [L4/TCP/UDP](section_05/ysj/note.md), [DNS와 port](section_12/ysj/note.md)
...
- [IANA - Service Name and Transport Protocol Port Number Registry](https://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.xhtml): TCP/UDP port 번호 범위와 공식 등록 현황
...
# Part 3. Host OS / 커널 / 표준 API

이 파트는 프로토콜 자체보다 **host 안에서 네트워크 기능이 어떻게 구현되고, 프로그램에게 어떤 API로 노출되는지**를 본다.
...
OS는 interface별로 MAC 주소, IP/prefix, MTU, up/down 상태를 관리하고, packet을 보낼 때 routing table과 ARP/ND cache를 사용한다.

- **Interface 상태**: `eth0`, `en0`, `lo` 같은 interface에 MAC, MTU, IP/prefix가 붙음
- **Routing table**: 목적지 IP를 보고 local 전달인지, default gateway로 보낼지 결정
- **ARP/ND cache**: next-hop IP에 대응하는 MAC 주소를 기억
- **송신 흐름**: socket data -> TCP/UDP -> IP packet -> routing lookup -> next-hop MAC 확인 -> L2 frame -> NIC
- **수신 흐름**: NIC가 frame 수신 -> IP packet 확인 -> protocol 필드에 따라 ICMP/TCP/UDP 등으로 전달

## OS의 TCP 처리 방식

TCP 상태는 **연결을 기다리는 socket**, **연결별 진행 상태**, **프로세스가 들고 있는 file descriptor**로 나누면 덜 겹친다.

**1. Listening socket 상태**
...
반대로 interface, routing table, ARP/ND cache처럼 **여러 연결이 공유하는 것**은 OS 네트워크 스택의 공통 기반이다.

## OS의 UDP 처리 방식

UDP는 TCP처럼 연결별 sequence/ACK/재전송 상태를 만들지 않는다. 그래도 OS는 UDP socket과 buffer는 관리한다.

- `bind()`된 UDP socket은 특정 local port로 들어오는 datagram을 받는다.
- 커널은 UDP header의 destination port를 보고 알맞은 socket receive queue에 datagram을 넣는다.
- `recvfrom()`은 byte stream이 아니라 datagram 단위로 읽는다.
- UDP `connect()`는 handshake가 아니라 기본 peer를 기록해 `send()/recv()`를 편하게 쓰는 의미에 가깝다.

## Socket API와 L4

프로그램은 보통 OS의 socket API를 통해 L4를 사용한다. (정글 코스 관련?)

여기선 C와 Linux(Glibc)의 그런거 사용한 소통 방식을 설명한다.
...
# Part 4. 유저 레벨 도구와 사용 흐름

이 파트는 구현보다 **사람이 어떤 명령어와 도구로 네트워크 상태를 확인하고, 애플리케이션 통신을 관찰하는지**에 초점을 둔다.
...
여러 계층과 관점이 섞여 있거나, 아직 어느 파트에 둘지 애매한 주제를 임시로 둔다.
