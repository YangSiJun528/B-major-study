...
L3는 서로 다른 L2 네트워크/서브넷 사이에서 **IP 패킷을 다음 홉으로 넘기는 계층**이다.

L3는 "목적지 IP가 어느 네트워크에 속하고, 다음 라우터/인터페이스가 어디인가"를 다룬다.
...
요약: **IP는 end-to-end 목적지, MAC은 hop-by-hop 다음 링크 목적지**
...
라우터나 L3 스위치는 프레임의 MAC이 아니라 **패킷의 목적지 IP**를 기준으로 다음 경로를 정한다.
...
- **traceroute**: probe(테스트 패킷을 지칭) 자체는 UDP/ICMP/TCP일 수 있지만, 중간 hop 확인은 보통 라우터의 `ICMP Time Exceeded` 응답을 이용
...
## 번외: Linux 기반 미니 PC를 라우터로 만드는 방법
...
**인상깊은 의견**
- 리눅스를 라우터로 만드는 일이 특수한 마법이 아니라, Docker/VM NAT 네트워크이나 Android hotspot에서 이미 쓰는 Linux 커널 네트워킹 기능을 직접 조립해 보는 것
...
- 내가 정리한 관련 섹션: [L3 기본과 subnet](section_06/ysj/note.md), [IP class 개념](section_07/ysj/note.md), [CIDR/subnetting/VLSM](section_08/ysj/note.md), [라우터/스위치 기본](section_11/ysj/note.md), [장비 관리 기초](section_14/ysj/note.md), [Routing](section_17/ysj/note.md), [ping/traceroute](section_18/ysj/note.md), [동적 라우팅 프로토콜](section_19/ysj/note.md), [OSPF](section_20/ysj/note.md), [Inter-VLAN Routing](section_22/ysj/note.md)
