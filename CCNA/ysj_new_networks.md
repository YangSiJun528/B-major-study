...
- `SYN`: 클라이언트가 초기 sequence number와 옵션을 보냄
- `SYN/ACK`: 서버가 자기 초기 sequence number와 ACK를 보냄
- `ACK`: 클라이언트가 확인 응답을 보내고 연결 성립
...
즉 3-way handshake는 단순 인사라기보다, 양쪽 커널이 신뢰성 있는 byte stream을 만들기 위한 **초기 상태를 맞추는 과정**이다. 이때 MSS, window scaling, SACK 같은 옵션도 협상할 수 있다.
...
- 클라이언트 쪽 `SYN-SENT`
  - 선택한 ephemeral port
  - 내가 보낸 초기 sequence number
  - 보낸 TCP option
  - SYN 재전송 timer
- 서버 쪽 `SYN-RECEIVED`
  - client IP/port
  - client의 초기 sequence number
  - 서버가 보낸 초기 sequence number
  - 협상 중인 TCP option
  - 마지막 ACK를 기다리는 timer

**4. Established connection 상태**
...
예를 들어 수신자가 byte `0~999`를 받았다면 ACK `1000`을 보낸다. 중간 segment가 빠지면 그 뒤 데이터가 도착해도 애플리케이션에는 바로 올리지 않고, 누락된 부분을 재전송받아 순서를 맞춘다.
...
프로그램은 직접 Ethernet frame이나 IP packet을 만드는 경우보다, 보통 OS의 socket API를 통해 L4를 사용한다.
...
CCNA에서 L4를 보는 이유는 "IP까지는 가는데 서비스가 열려 있는가", "어떤 트래픽을 허용/차단할 것인가"를 판단하기 위해서다.

- **ACL / firewall**: 출발지/목적지 IP뿐 아니라 protocol, port 기준으로 트래픽 허용/차단
- **Stateful firewall**: TCP state 또는 flow table을 추적해 요청에 대한 응답 트래픽을 허용
- **NAT/PAT**: 사설 IP를 공인 IP로 바꾸고, PAT는 port까지 사용해 여러 내부 host의 통신을 구분
- **Load Balancer**: L4 load balancer는 IP/port/connection 단위로 backend를 선택
- **Troubleshooting**: L3 도달성, L4 port 열림, L7 애플리케이션 응답을 분리해서 확인

예를 들어 `ping`은 되는데 `curl`은 실패한다면, IP 경로는 살아 있어도 TCP 443이 막혔거나 서버 프로세스가 없거나 TLS/HTTP 단계에서 실패했을 수 있다.
